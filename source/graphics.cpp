...
float Font::measureText(const std::u32string& text) const
{
    if(m_size > 0.f && !m_face.isNull())
...
void Canvas::fillText(const std::u32string& text, const Font& font, const Point& origin, const Transform& transform)
{
    plutovg_canvas_reset_matrix(m_canvas);
    plutovg_canvas_translate(m_canvas, -m_x, -m_y);
    plutovg_canvas_transform(m_canvas, &transform.matrix());
...
void Canvas::strokeText(const std::u32string& text, float strokeWidth, const Font& font, const Point& origin, const Transform& transform)
{
    plutovg_canvas_reset_matrix(m_canvas);
    plutovg_canvas_translate(m_canvas, -m_x, -m_y);
    plutovg_canvas_transform(m_canvas, &transform.matrix());
