...
public:
    ...
    bool parse(std::string input) final;
...
using SVGEnumerationEntry = std::pair<Enum, std::string>;
...
private:
    ...
    bool parseEnum(std::string input, const SVGEnumerationEntry<Enum>(&entries)[N]);
    Enum m_value;
};
...
public:
    ...
    bool parse(std::string input, LengthNegativeMode mode);
...
public:
    ...
    float value() const { return m_value; }
    bool parse(std::string input) override;
