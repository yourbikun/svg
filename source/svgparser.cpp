...
public:
    ...
    bool parseSheet(std::string input);
...
private:
    static bool parseRule(std::string& input, Rule& rule);
    static bool parseSelectors(std::string& input, SelectorList& selectors);
    static bool parseDeclarations(std::string& input, DeclarationList& declarations);
    static bool parseSelector(std::string& input, Selector& selector);
    static bool parseSimpleSelector(std::string& input, SimpleSelector& simpleSelector);

    RuleDataList m_rules;
    size_t m_position{0};
};

bool StyleSheet::parseSheet(std::string input)
{
    Rule rule;
    while(!input.empty()) {
        skipOptionalSpaces(input);
...
bool StyleSheet::parseRule(std::string& input, Rule& rule)
{
...
bool StyleSheet::parseSelectors(std::string& input, SelectorList& selectors)
{
    do {
        Selector selector;
 ...
 bool IS_CSS_STARTNAMECHAR(int c) { return IS_ALPHA(c) || c == '_' || c == '-'; }
 bool IS_CSS_NAMECHAR(int c) { return IS_CSS_STARTNAMECHAR(c) || IS_NUM(c); }

inline bool readCSSIdentifier(std::string& input, std::string& output)
{
...
bool StyleSheet::parseDeclarations(std::string& input, DeclarationList& declarations)
{
    if(!skipDelimiter(input, '{'))
        return false;
    skipOptionalSpaces(input);
    do {
        ...
        while(!input.empty() && !(input.front() == '!' || input.front() == ';' || input.front() == '}'))
            input.substr(1);
        value.erase(value.size() - input.length());
...
bool StyleSheet::parseSelector(std::string& input, Selector& selector)
{
    do {
...
bool StyleSheet::parseSimpleSelector(std::string& input, SimpleSelector& simpleSelector)
{
    std::string name;
    if(skipDelimiter(input, '*'))
...
inline void parseInlineStyle(std::string input, SVGElement* element)
{
    std::string name;
    std::string value;
    skipOptionalSpaces(input);
...
inline bool decodeText(std::string input, std::string& output)
{
    output.clear();
    while(!input.empty()) {
        auto ch = input.front();
 ...
 bool IS_STARTNAMECHAR(int c) { return IS_ALPHA(c) ||  c == '_' || c == ':'; }
 bool IS_NAMECHAR(int c) { return IS_STARTNAMECHAR(c) || IS_NUM(c) || c == '-' || c == '.'; }

inline bool readIdentifier(std::string& input, std::string& output)
{
...
bool Document::parse(const char* data, size_t length)
{
    ...
    int ignoring = 0;
    auto handleText = [&](const std::string& text, bool in_cdata) {
