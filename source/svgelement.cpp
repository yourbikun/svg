...
bool SVGElement::hasAttribute(const std::string& name) const
{
...
const std::string& SVGElement::getAttribute(const std::string& name) const
{
    auto id = propertyid(name);
    if(id == PropertyID::Unknown)
        return emptyString;
...
bool SVGElement::setAttribute(const std::string& name, const std::string& value)
{
    auto id = propertyid(name);
    if(id == PropertyID::Unknown)
        return false;
...
SVGMarkerElement* SVGElement::getMarker(const std::string& id) const
{
...
SVGClipPathElement* SVGElement::getClipper(const std::string& id) const
{
    auto element = rootElement()->getElementById(id);
...
SVGMaskElement* SVGElement::getMasker(const std::string& id) const
{
    auto element = rootElement()->getElementById(id);
...
SVGPaintElement* SVGElement::getPainter(const std::string& id) const
{
    auto element = rootElement()->getElementById(id);
...
SVGElement* SVGRootElement::getElementById(const std::string& id) const
{
...
static Bitmap loadImageResource(const std::string& href)
{
    if (href.compare(0, 5, "data:") == 0) {
        std::string input(href);
        auto index = input.find(',', 5);  // 从第 5 个字符开始查找逗号
        if (index == std::string::npos)
            return Bitmap();
        
        // 使用 substr 来移除前缀
        input = input.substr(index + 1);  // 从逗号之后的位置开始，获取剩余部分
