...
static Optional<Color> parseColorValue(std::string& input, const SVGLayoutState* state)
{
...
static Color parseColor(std::string input, const SVGLayoutState* state, const Color& defaultValue)
{
...
static Color parseColorOrNone(std::string input, const SVGLayoutState* state, const Color& defaultValue)
{
...
static bool parseUrlValue(std::string& input, std::string& value)
{
...
static std::string parseUrl(std::string input)
{
    std::string value;
...
static Paint parsePaint(std::string input, const SVGLayoutState* state, const Color& defaultValue)
{
    std::string id;
...
static float parseNumberOrPercentage(std::string input, bool allowPercentage, float defaultValue)
{
    float value;
...
static Length parseLength(const std::string& input, LengthNegativeMode mode, const Length& defaultValue)
{
    Length value;
...
static LengthList parseDashArray(std::string input)
{
    if(input.compare("none") == 0)
        return LengthList();
    LengthList values;
...
static float parseFontSize(std::string input, const SVGLayoutState* state)
{
...
static Enum parseEnumValue(const std::string& input, const SVGEnumerationEntry<Enum>(&entries)[N], Enum defaultValue)
{
...
static Display parseDisplay(const std::string& input)
{
...
static Visibility parseVisibility(const std::string& input)
{
...
static Overflow parseOverflow(const std::string& input)
{
...
static FontWeight parseFontWeight(const std::string& input)
{
...
static FontStyle parseFontStyle(const std::string& input)
{
...
static Direction parseDirection(const std::string& input)
{
...
static TextAnchor parseTextAnchor(const std::string& input)
{
...
static WhiteSpace parseWhiteSpace(const std::string& input)
{
...
static MaskType parseMaskType(const std::string& input)
{
...
static FillRule parseFillRule(const std::string& input)
{
...
static LineCap parseLineCap(const std::string& input)
{
...
static LineJoin parseLineJoin(const std::string& input)
{
    ...
    , m_font_family(parent.font_family())
{
    for(const auto& attribute : element->attributes()) {
        std::string input(attribute.value());
    ...
    while(!input.empty() && face.isNull()) {
        ...
        if(!family.empty() && (family.front() == '\'' || family.front() == '"')) {
            ...
            if(!family.empty() && family.back() == quote)
                family = family.substr(0, family.size() - 1);
            stripLeadingAndTrailingSpaces(family);
        }
