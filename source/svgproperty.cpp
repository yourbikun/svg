...
bool SVGString::parse(std::string input)
{
...
template<>
bool SVGEnumeration<SpreadMethod>::parse(std::string input)
{
...
template<>
bool SVGEnumeration<Units>::parse(std::string input)
{
...
template<>
bool SVGEnumeration<MarkerUnits>::parse(std::string input)
{
...
bool SVGEnumeration<Enum>::parseEnum(std::string input, const SVGEnumerationEntry<Enum>(&entries)[N])
{
    stripLeadingAndTrailingSpaces(input);
...
bool SVGAngle::parse(std::string input)
{
    stripLeadingAndTrailingSpaces(input);
    if(input == "auto") {
        m_value = 0.f;
...
bool Length::parse(std::string input, LengthNegativeMode mode)
{
    float value = 0.f;
    stripLeadingAndTrailingSpaces(input);
    if(!parseNumber(input, value))
...
bool SVGLength::parse(std::string input)
{
    return m_value.parse(input, m_negativeMode);
}

bool SVGLengthList::parse(std::string input)
{
    m_values.clear();
    while(!input.empty()) {
        size_t count = 0;
...
bool SVGNumber::parse(std::string input)
{
    float value = 0.f;
    stripLeadingAndTrailingSpaces(input);
    if(!parseNumber(input, value))
...
bool SVGNumberPercentage::parse(std::string input)
{
    float value = 0.f;
    stripLeadingAndTrailingSpaces(input);
    if(!parseNumber(input, value))
...
bool SVGNumberList::parse(std::string input)
{
    m_values.clear();
...
bool SVGPath::parse(std::string input)
{
    return m_value.parse(input.data(), input.length());
}

bool SVGPoint::parse(std::string input)
{
    Point value;
    stripLeadingAndTrailingSpaces(input);
...
bool SVGPointList::parse(std::string input)
{
    m_values.clear();
    stripLeadingSpaces(input);
    while(!input.empty()) {
...
bool SVGRect::parse(std::string input)
{
    Rect value;
    stripLeadingAndTrailingSpaces(input);
    if(!parseNumber(input, value.x)
...
bool SVGTransform::parse(std::string input)
{
    return m_value.parse(input.data(), input.length());
}

bool SVGPreserveAspectRatio::parse(std::string input)
{
